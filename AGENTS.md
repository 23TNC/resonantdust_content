...
content/
  ...
  src/
    ...
    wasm_api.rs          # JS-facing wasm-bindgen exports
                         #   (cfg(feature = "js"))
  cards/
    types.json         # card_type / card_category registry, with
                       #   per-type visibility + shape
    ...
    flags.json         # bit-position registry for cards.flags
                       #   (also actions.flags / magnetic_actions.flags
                       #    sections, currently dormant)
...
| [`cards/flags.json`](cards/flags.json) | live | Bit-position registry for the `cards.flags` u32 column (also has `actions` / `magnetic_actions` sections, currently dormant). Two entry shapes: `"bit": n` for single-bit flags, `"bits": [low, …, high]` for contiguous multi-bit fields (e.g. `progress_style` at bits 8..=10, `position_hold_count` at 17..=19). Append-only — removed flags leave `_reserved_<bit>` tombstones. Both the server and the recipe parser (for `{"flag": "..."}` predicates) read this file via [`flags_core`](src/flags_core.rs). |
...
| [`recipes/types.json`](recipes/types.json) | live | `recipe_type` / `recipe_category` id registry, used by `pack_recipe`. Types: `stack`, `on_create`, `magnetic`. Categories: `up`, `down`, `self`, `magnetic`. `valid_for` lists which (type, category) pairs the loader recognises. |
...
- [`src/definition_core.rs`](src/definition_core.rs) — `build_aspects`,
  ...
  `is_hex_type`, `card_locale_path`, `card_type_ids`,
  `card_category_ids`, and the `ASPECT_NONE` / `TRAIT_NONE` sentinels.
